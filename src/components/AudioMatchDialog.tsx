...
export default function AudioMatchDialog({
  open,
  onClose,
}: {
  open: boolean;
  ...
  return (
    ...
    >
      <div
        ...
        className={`modal audio-match-dialog ${transition.surfaceClassName}`}
        role="dialog"
        aria-modal="true"
      >
        <header className="modal-heading">
          <h2>
            <Fingerprint size={18} /> 音频识别
          </h2>
        ...
        <label className="field-label">
          ...
          <textarea
            value={audioFP}
            rows={4}
            placeholder="粘贴 audioFP"
            onChange={(event) => setAudioFP(event.target.value)}
          />
        </label>
        <label className="field-label">
          时长（毫秒）
