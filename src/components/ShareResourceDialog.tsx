...
export default function ShareResourceDialog({ song, open, onClose }: { song: Song | null; open: boolean; onClose: () => void }) {
  ...
  const transition = useOriginTransition<HTMLElement>(open, "player-share", 220);
  ...
  return (
    <div className={`modal-backdrop share-dialog-backdrop ${transition.backdropClassName}`} onClick={onClose}>
      <section ref={transition.surfaceRef} className={`modal share-dialog ${transition.surfaceClassName}`} role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}>
