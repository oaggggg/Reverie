...
export default function LibraryPage() {
  ...
  useEffect(() => {
    ...
    void (tab === "albums" ? loadAlbums() : loadArtists())
      .catch(() => {
        if (alive)
          usePlayerStore.getState().toast("加载音乐馆内容失败", "error");
      })
      .finally(() => {
  ...
  return (
    <Page>
      ...
      <div className="library-filters">
        {tab === "albums" ? (
          ...
          <>
            <select
              value={artistType}
              onChange={(event) => setArtistType(Number(event.target.value))}
            >
              <option value={1}>男歌手</option>
              <option value={2}>女歌手</option>
              <option value={3}>乐队</option>
            </select>
            <div className="library-initials">
              {INITIALS.map((item) => (
                <button
                  key={item}
                  className={
                    initial === (item === "全部" ? "" : item) ? "active" : ""
                  }
                  onClick={() => setInitial(item === "全部" ? "" : item)}
                >
                  {item}
                </button>
              ))}
      ...
      {loading && !(tab === "albums" ? albums.length : artists.length) ? (
        <LoadingState label="正在加载音乐馆…" />
      ) : tab === "albums" ? (
        <div className="media-grid compact">
          ...
          {!albums.length && <div className="empty">暂无专辑</div>}
        </div>
      ) : (
        <div className="search-person-grid">
          {artists.map((artist) => (
            <button key={artist.id} onClick={() => void openArtist(artist.id)}>
              {artist.picUrl ? (
                <img src={sizedImage(artist.picUrl, 240)} alt="" />
              ) : (
          ...
          {!artists.length && <div className="empty">暂无歌手</div>}
        </div>
      )}
